...
public class ShapeUtilities
{
    ...
    public static Point2D.Double getClickedPointInObjectCoords(Point2D.Double pt, Shape shape)
    {
        ...
        //AffineTransform worldToObj = viewToWorld;
        AffineTransform worldToObj = new AffineTransform();

        double theta = -Math.toRadians(shape.getRotation());
        double m00 = Math.cos(theta); double m01 = -Math.sin(theta); double m02 = 0;
        double m10 = Math.sin(theta); double m11 = Math.cos(theta);  double m12 = 0;

        AffineTransform rotMatrix = new AffineTransform(m00,m10, m01, m11,m02,m12); //makes a matrix

        worldToObj.concatenate(rotMatrix);

        Point2D.Double c = shape.getCenter();
        m00 = 1;    m01 = 0;    m02 = -c.x;
        m10 = 0;    m11 = 1;    m12 = -c.y;

        AffineTransform translationMatrix = new AffineTransform(m00,m10, m01, m11,m02,m12);

        worldToObj.concatenate(translationMatrix);



        //LAB 3 VIEWPORT TO WORLD_______________________________________________________________________________________________
        ...
        worldToObj.concatenate(scaleWorldToViewMatrix);
